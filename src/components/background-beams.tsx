...
const BackgroundBeams = memo(({ className }: BackgroundBeamsProps) => {
  return (
    <div className={cn("pointer-events-none absolute inset-0 -z-10 h-full w-full", className)}>
      <svg
        aria-hidden="true"
      ...
      >
        <g opacity="0.03">
          {pathData.map((d, i) => (
            <path d={d} key={`static-${i}`} stroke="white" strokeWidth="0.5" />
          ))}
        </g>
        {pathData.map((d, i) => (
          <motion.path
        ...
        <defs>
          {pathData.map((_, i) => (
            <linearGradient id={`gradient-${i}`} key={`gradient-${i}`} x1="0%" x2="100%" y1="0%" y2="100%">
